...
def get_cell_labels(img, threshold):
    ...
    for x in range(len(img)):
        for y in range(len(img[x])):
            if x < 399 and img[(x,y)] > threshold and img[(x+1, y)] > threshold:
                u.union((x,y), (x+1, y))
            if x > 0 and img[(x,y)] > threshold and img[(x-1, y)] > threshold:
                u.union((x,y), (x-1, y))
            if y < 399 and img[(x,y)] > threshold and img[(x, y+1)] > threshold:
                u.union((x,y), (x, y+1))
            if y > 0 and img[(x,y)] > threshold and img[(x, y-1)] > threshold:
                u.union((x,y), (x, y-1))
